...
class HabitLocalService {
  ...
  Future<void> saveCompletion(HabitCompletionModel completion) async {
    await _completionBox.put(
      '${completion.habitId}_${completion.completedDate}',
      completion.toJson(),
    );
  }
  ...
  Future<void> markAsSynced(String habitId, String date) async {
    ...
    if (json != null) {
      final completion = HabitCompletionModel.fromJson(
  ...
  List<HabitCompletionModel> getCompletionsForDateRange(
    String startDate,
    String endDate,
  ) {
    return _completionBox.values
        ...
        .where(
          (c) =>
              c.completedDate.compareTo(startDate) >= 0 &&
              c.completedDate.compareTo(endDate) <= 0,
        )
        .toList();
  }
  ...
  List<HabitCompletionModel> getRecentCompletions(int limit) {
    ...
    completions.sort((a, b) {
      final dateA =
          a.completedAt ?? DateTime.tryParse(a.completedDate) ?? DateTime.now();
      final dateB =
          b.completedAt ?? DateTime.tryParse(b.completedDate) ?? DateTime.now();
